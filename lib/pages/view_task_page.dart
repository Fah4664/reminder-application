...
class ViewTasksPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title:
            const Center(child: Text('View Completed Tasks')), // AppBar title
      ...
      body: Consumer<TaskProvider>(
        ...
        builder: (context, taskProvider, child) {
          final completedTasks =
              taskProvider.completedTasks; // Get completed tasks from provider
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final task =
                  completedTasks[index]; // Get the task at the current index
              return Card(
                ...
                margin: const EdgeInsets.symmetric(
                    vertical: 10, horizontal: 15), // Margin around card
                child: ListTile(
                  title: Column(
                    ...
                    children: [
                      ...
                      Text(
                        formatDateRange(task.startDateTime,
                            task.endDateTime), // Show date range
                        style: const TextStyle(
                            fontSize: 16,
                            color: Color(0xFF000000)), // Date style
                      ),
                      const SizedBox(height: 5), // Spacer
                      Container(
                        ...
                        child: Align(
                          alignment: Alignment
                              .centerLeft, // Align progress indicator to the left
                          child: Container(
                            width: (MediaQuery.of(context).size.width - 100) *
                                (task.sliderValue *
                                    100 /
                                    150), // Calculate width based on slider value
                            height: 15, // Height of the inner progress bar
                            decoration: BoxDecoration(
                              color:
                                  const Color(0xFF717273), // Progress bar color
      ...
      bottomNavigationBar: BottomAppBar(
        ...
        child: Row(
          mainAxisAlignment:
              MainAxisAlignment.spaceBetween, // Space out navigation buttons
          children: <Widget>[
            const SizedBox(width: 48), // Empty space on the left
            IconButton(
              icon: Image.asset(
                'assets/icons/target.png', // Icon for home page
                height: 27,
                width: 27,
              ),
              onPressed: () {
            ...
            IconButton(
              icon: Image.asset(
                'assets/icons/Add Task.png', // Icon for adding a task
                height: 29,
                width: 29,
              ),
              onPressed: () {
            ...
            IconButton(
              icon: Image.asset(
                'assets/icons/search.png', // Icon for search page
                height: 28,
                width: 28,
              ),
              onPressed: () {
